...
WSL 터미널을 열고 아래 명령을 순서대로 실행하면 끝입니다.
...
> `git clone` 이 폴더를 자동으로 만들어주므로 별도로 폴더를 만들 필요가 없습니다.

**3줄 실행으로 자동으로 세팅되는 것들:**

| 항목 | 내용 |
|------|------|
| PHP 8.1 + Apache | Dockerfile 기반으로 자동 빌드 |
| MySQL 8.0 | 컨테이너 자동 실행 |
| DB + 테이블 생성 | `init.sql` 자동 실행 |
| 샘플 데이터 삽입 | `init.sql` 안에 포함 |
